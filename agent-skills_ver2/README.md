...
<details>
<summary><b>Cursor（推奨 — プリセット済み）</b></summary>

agent-skills2 は Cursor 向けに **そのまま使える** 状態です。
...
含まれるもの:
- `.cursor/rules/` — オーケストレーション + フェーズ別ルール（12個）
- `.cursor/commands/` — `/spec`, `/plan`, `/build`, `/test`, `/review`, `/code-simplify`, `/ship`, `/webperf`

Agent チャットで `/` を入力するとコマンド一覧が表示されます。詳細は [docs/cursor-setup.md](docs/cursor-setup.md)。
