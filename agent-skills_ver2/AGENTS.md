...
**agent-skills2** は upstream の agent-skills に Cursor 向けプリセット（`.cursor/rules/`, `.cursor/commands/`）を追加した派生版です。

## Cursor Integration

Cursor では `.cursor/rules/` と `.cursor/commands/` が自動読み込みされます。
...
| `/build auto` | above + `planning-and-task-breakdown` |
| `/test` | `test-driven-development` |
| `/review` | `code-review-and-quality` |
| `/code-simplify` | `code-simplification` |
| `/ship` | `shipping-and-launch` + `agents/*` |
| `/webperf` | `web-performance-auditor` |

### Session Hook Equivalent

Claude Code の `session-start` フック（`using-agent-skills` メタスキル注入）に相当するのは:

- `.cursor/rules/00-orchestration.mdc` — スキル発見・振り分け
- `.cursor/rules/01-core-behaviors.mdc` — 共通行動規範

これらは `alwaysApply: true` で常時有効です。

### Install to Another Project

```bash
./scripts/install-to-cursor-project.sh /path/to/project
```

See [docs/cursor-setup.md](docs/cursor-setup.md) for full details.
